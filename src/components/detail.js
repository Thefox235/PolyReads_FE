...
const Detail = () => {
  ...
  const limit = 10; // Số comment trên mỗi trang
  ...
  // Ví dụ: gọi API để lấy bình luận cho sản phẩm với paging
  ...
  const fetchComments = async (page = 1, limit = 10) => {
    try {
      const data = await getComment({ productId: id, page, limit });
      // data có thể chứa { comments, total, page, limit }
      // console.log(data);
  ...
  useEffect(() => {
    if (id) {  // chỉ gọi khi id có giá trị
      // console.log("Fetching comments with productId:", id);
      fetchComments();  // fetchComments truyền productId, page, limit
    }
  }, [id]);
  //chuyển tab
  ...
  const displayedComments = activeFilter === 0
    ? comments
    : comments.filter((item) => Number(item.rating) === activeFilter);
  ...
  const checkUserPurchasedProduct = async (userId, productId) => {
    try {
      ...
      const ordersWithDetails = completedOrders.map((order, index) => ({
        order,
        orderDetails: Array.isArray(ordersDetailsArray[index].orderDetails)
      ...
      for (const item of ordersWithDetails) {
        ...
        if (purchasedInThisOrder) {
          // Trả về đơn hàng mà sản phẩm được mua
          return item.order;
        }
      }
      // Nếu không tìm thấy đơn hàng nào phù hợp, trả về null
      return null;
    } catch (error) {
      console.error("Error checking purchased product:", error);
      return null;
    }
  };


  const handleCommentClick = async (productId) => {
    const user = JSON.parse(sessionStorage.getItem("user"));
    if (!user) {
      alert("Bạn cần đăng nhập để bình luận!");
      return;
    }
    // Lấy đơn hàng mà sản phẩm đã được mua
    const purchasedOrder = await checkUserPurchasedProduct(user._id, productId);
    if (!purchasedOrder) {
      alert("Bạn chưa mua sản phẩm này hoặc đơn hàng của bạn chưa hoàn thành nên không được đánh giá.");
      return;
    }
    // Lưu orderId (bạn có thể lưu vào state hoặc truyền được qua prop)
    setCurrentOrderId(purchasedOrder._id); // ví dụ bạn có state currentOrderId
            ...
            >
              ...
              {displayedComments.map((item) => (
                <div className="review" key={item._id}>
                  <div className="review-header">
                    ...
                    <img src={item.userId.url_image} alt="avatar" className="review-avatar" />
                  </div>
              ...
              <div className="m-auto w-100">
                <div className="paginations">
                  <button
                    onClick={() => currentPage > 1 && fetchComments(currentPage - 1)}
                  ...
                  <button
                    onClick={() => currentPage < totalPages && fetchComments(currentPage + 1)}
